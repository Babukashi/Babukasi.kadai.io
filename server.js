...
async function handler(request) {
  ...
  if (request.method === 'POST') {
    ...
    if (pathname === '/api/login') {
      console.log('--- ログイン試行 ---');
      console.log('入力されたID:', body.id);
      console.log('入力されたPW:', body.password);
      ...
      if (user) {
        ...
        console.log('結果: 失敗 (IDまたはPW不一致)');
        return jsonResponse({ message: 'IDまたはパスワードが違います' }, 401);
      }
    }
    // ... その他のAPI（add-bookmark, update-profile等）は前回と同じ
    if (pathname === '/api/signup') {
      if (MOCK_USERS.find((u) => u.id === body.id)) return jsonResponse({ message: 'このIDは既に使われています' }, 400);
